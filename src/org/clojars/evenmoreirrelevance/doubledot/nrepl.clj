             ...
             (when-let [^Class the-class (try (Class/forName
               ...
               (concat
                ...
                (sequence (comp (filter (fn starts-well [[mn _]] (str/starts-with? mn sym-name)))
                                (keep (fn some-member-accessible [[_ ^..reflect.Member ms]]
                                        (rfirst
                                         (filter #(.isAssignableFrom (.getDeclaringClass ^..reflect.Member %) the-class))
                                         ms)))
                                (map (fn mem->candidate [^..reflect.Member m]
 ...
 (defn ^:private real-defsource
   ...
   (let [out (compliment-defsource name source)]
     (alter-var-root clj-sources
                     #(cond-> %
                        (= -1 (.indexOf ^..util.List % name)) (conj name)))
     out))
